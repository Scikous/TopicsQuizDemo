...
const getCorrectAnswer = async (questionID) => {
  try {
    const answerText =
      await sql`SELECT option_text FROM question_answer_options WHERE is_correct=true AND question_id=${questionID}`;
...
export {
  getRandQuestionByTopicID,
  getCorrectAnswer,
  addUserAnswer,
  getAnswerTFValue,
  getRandQuestionAsJSON,
};
