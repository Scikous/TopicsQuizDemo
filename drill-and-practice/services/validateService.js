...
const topicExistsByName = async (topic) => {
  try {
    const exists =
      await sql`SELECT EXISTS (SELECT 1 FROM topics WHERE name=${topic})`;
...
const questionExistsByName = async (question, topicID) => {
  try {
    const exists =
      await sql`SELECT EXISTS (SELECT 1 FROM questions WHERE question_text=${question} AND topic_id=${topicID})`;
    return exists[0].exists;
  } catch (e) {
...
const questionAOExistsByName = async (questionAO, questionID) => {
  try {
    const exists =
      await sql`SELECT EXISTS (SELECT 1 FROM question_answer_options WHERE option_text=${questionAO} AND question_id=${questionID})`;
    return exists[0].exists;
  } catch (e) {
...
export {
  topicExistsByName,
  questionExistsByName,
  questionAOExistsByName,
  userExists,
};
